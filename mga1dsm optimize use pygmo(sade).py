...
def convert_trajectory_parameters(
    ...
    trajectory_parameters: List[float],
     ...
     # Extract node times
    node_times = trajectory_parameters[:5]

    # 提取leg_free_parameters参数
    number_of_leg_free_params = 4  # 假设有4个leg_free_parameters参数
    leg_free_parameters_start_index = 5
    ...
    number_of_nodes_minus_one = 5 - 1  # 假设每个节点（除了最后一个）有3个参数
...
class TransferTrajectoryProblem:
    ...
    def get_bounds(self) -> tuple:
        ...
        lower_bound = [self.departure_date_lb, self.departure_date_lb + self.legs_tof_lb[0], self.departure_date_lb + self.legs_tof_lb[0] + self.legs_tof_lb[1], self.departure_date_lb + self.legs_tof_lb[0] + self.legs_tof_lb[1] + self.legs_tof_lb[2], self.departure_date_lb + self.legs_tof_lb[0] + self.legs_tof_lb[1] + self.legs_tof_lb[2] + self.legs_tof_lb[3],
                       self.yita_lb, self.yita_lb, self.yita_lb, self.yita_lb,
                       self.sudu_lb, self.u_lb, self.v_lb, self.pe_lb * 6.052e6, self.jiao_lb, 0.0, self.pe_lb * 6.378e6, self.jiao_lb, 0.0, self.pe_lb * 6.378e6, self.jiao_lb, 0.0]
        upper_bound = [self.departure_date_ub, self.departure_date_ub + self.legs_tof_ub[0], self.departure_date_ub + self.legs_tof_ub[0] + self.legs_tof_ub[1], self.departure_date_ub + self.legs_tof_ub[0] + self.legs_tof_ub[1] + self.legs_tof_ub[2], self.departure_date_ub + self.legs_tof_ub[0] + self.legs_tof_ub[1] + self.legs_tof_ub[2] + self.legs_tof_ub[3],
                       self.yita_ub, self.yita_ub, self.yita_ub, self.yita_ub,
                       self.sudu_ub, self.u_ub, self.v_ub, self.pe_ub * 6.052e6, self.jiao_ub, 0.0, self.pe_ub * 6.378e6, self.jiao_ub, 0.0, self.pe_ub * 6.378e6, self.jiao_ub, 0.0]

        #整个问题的难点在这里，这里定义trajectory_parameters里每一个元素的上下界，6.052e6是venus半径，6.378e6是地球半径
...
time5 = time4 + 180.510754824 * julian_day

# 
leg1_param = [0.234594654679]
leg2_param = [0.0964769387134]
leg3_param = [0.829948744508]
leg4_param = [0.317174785637]

#
node1_param = [1408.99421278, 0.37992647165 * 2.0 * np.pi, np.arccos(2.0 * 0.498004040298 - 1.0) - np.pi / 2.0]
node2_param = [1.80629232251 * 6.052e6, 1.35077257078, 0.0]
node3_param = [3.04129845698 * 6.378e6, 1.09554368115, 0.0]
node4_param = [1.10000000891 * 6.378e6, 1.34317576594, 0.0]
node5_param = []

# 
trajectory_parameters = [
    time1, time2, time3, time4, time5,
    *leg1_param, *leg2_param, *leg3_param, *leg4_param,
    *node1_param, *node2_param, *node3_param, *node4_param, *node5_param
]
...
departure_date_lb = DateTime(2025, 4, 6).epoch()
departure_date_ub = DateTime(2026, 12, 31).epoch()
...
print("Earth-Venus time of flight [days]: ", best_decision_variables[1]-best_decision_variables[0])
print("Venus-Earth time of flight [days]: ", best_decision_variables[2]-best_decision_variables[1])
print("Earth-Earth time of flight [days]: ", best_decision_variables[3]-best_decision_variables[2])
print("Earth-Jupiter time of flight [days]: ", best_decision_variables[4]-best_decision_variables[3])

print('Delta V Timming per leg: ')
print(' - between Earth and Venus:',pop.champion_x[5] * (best_decision_variables[1]-best_decision_variables[0]))
...
'''
#分着画
# Determine the start and end index for the first segment
# Assuming the first segment starts at the first node time and ends at the second node time
start_index = np.where(state_history[:, 0] == node_times[0])[0][0]
end_index = np.where(state_history[:, 0] == node_times[1])[0][0]

# Extract the first segment of the state history
first_segment_state_history = state_history[start_index:end_index+1]

# Plot the first segment of the state history
fig = plt.figure(figsize=(8, 5))
ax = fig.add_subplot(111)
ax.plot(first_segment_state_history[:, 1] / au, first_segment_state_history[:, 2] / au)

# Scatter plot for the Earth departure (start of the first segment)
ax.scatter(
    fly_by_states[0, 0] / au,
    fly_by_states[0, 1] / au,
    color="blue",
...
# Scatter plot for the end of the first segment (Venus fly-by)
ax.scatter(
    fly_by_states[1, 0] / au,
    fly_by_states[1, 1] / au,
    color="green",
...
start_index = np.where(state_history[:, 0] == node_times[1])[0][0]
end_index = np.where(state_history[:, 0] == node_times[2])[0][0]

# Extract the first segment of the state history
second_segment_state_history = state_history[start_index:end_index+1]

# Plot the first segment of the state history
fig = plt.figure(figsize=(8, 5))
ax = fig.add_subplot(111)
ax.plot(second_segment_state_history[:, 1] / au, second_segment_state_history[:, 2] / au)
